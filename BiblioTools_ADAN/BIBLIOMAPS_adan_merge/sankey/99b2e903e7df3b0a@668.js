...
SankeyChart({
  ...
  format: (f => d => `${f(d)} Link`)(d3.format(",.1~f")),
  width,
  height: 600
})
)}
...
FileAttachment("energy.csv").csv({typed: true})
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["energy.csv", {url: new URL("./files/data.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
